...
* поведенияе конструкции `apply` и всех конструкций, использующих `apply`(`applyCtx` и т.д.)
...
* Базовый шаблон реализует логику, специфическуя для конкретной технологии:
	* набор стандартных мод и полей контекста;
	* поведение конструкции `apply` (`applyNext`) в соответствие с задачами технологии (генерация HTML или BEMJSON).
...
Как следствие использования такой архитектуры, синтаксис обоих шаблонизаторов почти идентичен. Единственная разница между синтаксисом BEMHTML и BEMTREE заключается в:
* количестве доступных стандартных мод;
* нескольких полях контекста, специфических для каждого из шаблонизаторов.


Общие для BEMHTML и BEMTREE синтаксические конструкции и поля контекста объединены понятием **унифицированнй синтаксис шаблонов**. 

Для сокращенного обозначения унифицированного синтаксиса шаблонов используется акроним UTS – аббревиатура от латинского Unified Template Syntax.

Шаблонизаторы, поддерживающие синтаксические конструкции и поля контекста, соответствующие UTS, – называются **UTS-шаблонизаторами**.

<a name="objects"></a>
###Глобальные объекты BEMHTML и BEMTREE
...
<a name="uts_basic"></a>
###Особенности UTS-шаблонизаторов
...
Поскольку UTS-шаблонизаторы основаны на JavaScript, в качестве формата представления БЭМ-дерева выбран JSON с набором дополнительных соглашений о представлении БЭМ-сущностей — BEMJSON.
...
Особый статус имеет **[мода default](#default)**, которая отвечает за генерацию результата вцелом для текущей БЭМ-сущности. В рамках этой
...
В UTS предусмотрены следующие типы условий:
