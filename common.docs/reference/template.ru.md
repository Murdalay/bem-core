...
Под **шаблонизацией** в bem-core понимается процесс генерации из некоторых входных данных результирующего HTML.

Средства и инструменты, позволяющие осуществлять такое преобразование, называются **механизмом шаблонизации**.


В `bem-core` реализован механизм двухэтапной шаблонизации данных. На первом этапе нормализованные данные, получаемых от бэкенда, преобразуются к вью-ориентированному представлению.  Эту функцию выполняет шаблонизатор [BEMTREE](http://ru.bem.info/libs/bem-core/2.1.0/bemtree/reference/). 
...
Базовый шаблон – это реализация блока `i-bem` в технологии  соответствующего шаблонизатора. Для шаблонизатора BEMTREE базовым шаблоном является реализация блока `i-bem` в технологии BEMTREE – [i-bem.bemtree](https://github.com/bem/bem-core/blob/v1/common.blocks/i-bem/i-bem.bemtree). 
Для шаблонизатора BEMHTML  – [i-bem.bemhtml](https://github.com/bem/bem-core/blob/v1/common.blocks/i-bem/i-bem.bemhtml).
...
* Базовый шаблон реализует логику, специфическуя для конкретной технологии:
...
![levels](https://raw.githubusercontent.com/Murdalay/bem-core/feature_doc/common.docs/reference/template_levels.png)

<a name="bemx_intro"></a>
####Понятие BEM-XJST-шаблонизаторов

Как следствие использования описанной выше архитектуры, синтаксис обоих шаблонизаторов почти идентичен. Единственная разница между синтаксисом BEMHTML и BEMTREE заключается в:
* количестве доступных стандартных мод;
* нескольких полях контекста, специфических для каждого из шаблонизаторов.


Общие для BEMHTML и BEMTREE синтаксические конструкции и поля контекста объединены понятием **синтаксис BEM-XJST-шаблонов**. 

Шаблонизаторы, поддерживающие синтаксические конструкции и поля контекста, соответствующие синтаксису BEM-XJST-шаблонов, – называются **BEM-XJST-шаблонизаторами**.


<a name="polymorph"></a>
###Сборка и подключение бандла технологий BEMTREE и BEMHTML

В соответствие с [БЭМ методологией](http://ru.bem.info/method/) реализации шаблонов BEMHTML и BEMTREE размещаются в [файловой системе](http://ru.bem.info/method/filesystem/) как набор файлов с суффиксами `.bemhtml.js` и .`bemtree.js`, соответственно.  

Файлы шаблонов размещаются в каталогах блоков, использующих шаблонизацию, а также в каталогах элементов.

Для того, чтобы использовать BEMHTML или BEMTREE шаблоны блоков и элементов, производится их **сборка** в файл бандла технологии. Суффикс собираемого бандла соответствует суффиксу файлов шаблонов, из которых он был собран.

 Средства и инструменты, с помощью которых осуществляется сборка, называются **технологией сборки**.
 
В `bem-core` для этих целей используется технология сборки с помощью пакета [bem-tools](http://ru.bem.info/tools/bem/bem-tools/).

Например, в [project-stub](https://github.com/bem/project-stub), с поддержкой `bem-core` командой `bem server` будета автоматически производиться сборка `.bemhtml.js` и `.bemtree.js` файлов из блоков со всех [уровней переопределения](http://ru.bem.info/tools/bem/bem-tools/levels/), используемых по умолчанию.

Файлы технологий из блоков, по умолчанию, собираются в каталоге `index` на уровне переопределения  `desktop.bundles`.

Для BEMHTML-шаблонов в каталоге создается файл бандла `index.bemhtml.js`. Для BEMTREE-шаблонов – `index.bemtree.js`.  

Помимо сборки и минимизации кода, модуль технологии, осуществляющий сборку, добавляет в файл бандла полифильную  обвязку, которая:
1. Позволяет подключить бандл как [ymodule](https://github.com/ymaps/modules);
2. Позволяет подключить бандл как CommonJS-модуль;
3. Формирует из бандла глобальный объект, в случает если ни один из пердыдущих способов не был использован.

####Использования глобальных объектов BEMHTML и BEMTREE

Выполнение технологии начинается с вызова метода `apply` из свойств соответствующего глобального объекта:
* `BEMHTML.apply()` для технологии BEMHTML;
* `BEMTREE.apply()` для технологии BEMTREE.

В качестве аргумента метод принимает фрагмент БЭМ-дерева, для которого осуществляется шаблонизация, в формате BEMJSON.

В упрощенном виде двухэтапная шаблонизация может быть реализована так:

```js
var data = BEMTREE.apply({ block: 'intitial' });
BEMHTML.apply(data)
```
...
**NB:** Методы глобальных объектов BEMHTML и BEMTREE могут быть вызваны в любом JavaScript-окружении, как на сервере, так и на клиенте. 
...
Поскольку BEM-XJST-шаблонизаторы основаны на JavaScript, в качестве формата представления БЭМ-дерева выбран JSON с набором дополнительных соглашений о представлении БЭМ-сущностей — BEMJSON.
...
* [Унифицированный синтаксис шаблонов](#unity)
...
Для вложенных сущностей цикл генерация результата выполняется рекурсивно. Таким образом, результат формируется поэлементно в процессе обхода входного БЭМ-дерева.
...
### Унифицированный синтаксис шаблонов
...
В процессе работы шаблонизаторы строят структуру данных, содержащую сведения об обрабатываемом узле BEMJSON и о состоянии процесса обработки. Помимо этого в контексте доступен ряд вспомогательных функций.
