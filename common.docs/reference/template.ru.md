...
Как следствие использования описанной выше архитектуры, синтаксис BEMHTML и BEMTREE почти идентичен. Вся разница между синтаксе шаблонизаторов  заключается в:
...
В терминах BEM-XJST-шаблонизации BEMJSON можно рассматривать как объект, асоциированный с полем [контекста](#context) `this.ctx`. 
...
Чтобы использовать BEMHTML или BEMTREE шаблоны блоков и элементов, производится их **сборка** в файл **бандла технологии**. Суффикс собираемого бандла соответствует суффиксу файлов шаблонов, из которых он был собран.
...
3. Формирует из бандла глобальный объект, в случае, если ни один из предыдущих способов не был использован.
...
Первым параметром методу передается массив строк – имен подключаемых модулей. Вторым параметром – колбек в который, после разрешения зависимостей, передаются объекты соответствующих модулей.
...
####BEMHTML и BEMTREE в виде глобальных объектов

Глобальный объекты BEMHTML или BEMTREE будет доступен в случае, если бандл соответствующей технологии не был подключен с помощью YModules или как CommonJS-модуль.

Методы глобальных объектов BEMHTML и BEMTREE могут быть вызваны в любом JavaScript-окружении, как на сервере, так и на клиенте. 


####Метод apply объектов BEMHTML и BEMTREE

В свойствах объектов BEMHTML и BEMTREE, будь то глобальные объекты или подключенные модули, есть метод `apply`:
* `BEMHTML.apply()` для технологии BEMHTML;
* `BEMTREE.apply()` для технологии BEMTREE.

Метод `apply` инициирует процесс шаблонизации.
В качестве аргумента он принимает фрагмент БЭМ-дерева, для которого осуществляется шаблонизация, в формате BEMJSON.

Метод `apply` объекта BEMTREE – асинхронный. Резульатом его выполнения будет промис. По этому, чтобы получить сформированный BEMJSON для следующего этапа шаблонизации, нужно воспользоваться методом `.then`. Этот метод дожидается выполнения промиса, а затем передает результат колбеку аргументом.
