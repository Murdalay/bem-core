...
В соответствие с [БЭМ методологией](http://ru.bem.info/method/) реализации шаблонов BEMHTML и BEMTREE размещаются в [файловой системе](http://ru.bem.info/method/filesystem/) как набор файлов с суффиксами `.bemhtml` и `.bemtree`, соответственно.  
...
Глобальный объекты BEMHTML будет доступен в случае, если бандл `.bemhtml.js`не был подключен с помощью YModules или как CommonJS-модуль.
...
В свойствах объектов BEMHTML и BEMTREE, вне зависимости от способа подключения бандла, есть метод `apply`:
...
Метод `apply` объекта BEMTREE – асинхронный. Резульатом его выполнения будет [vow](https://github.com/dfilatov/vow)-промис. По этому, чтобы получить сформированный BEMJSON для следующего этапа шаблонизации, нужно воспользоваться методом `.then`. Этот метод дожидается разрешения промиса, а затем передает результат аргументом в колбек.
