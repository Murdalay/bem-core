...
Задача BEMHTML — преобразовать входное БЭМ-дерево в выходной HTML-документ. 

Поэтому структура входного БЭМ-дерева должна быть максимально ориентирована на **представление** (view), чтобы при генерации
HTML-дерева не требовалось изменять набор и порядок блоков и элементов. Приведение БЭМ-дерева к такому развернутому виду и является задачей технологии BEMTREE. 
...
Тогда, в BEMTREE-шаблон для БЭМ-сущности, являющейся точкой входа, включаются ссылки на другие БЭМ-сущности. В ходе выполнения шаблона точки входа, шаблонизатор рекурсивно вызывает BEMTREE-шаблоны, на которые он ссылался. 
...
Таким образом, в BEMTREE-шаблонах доступны только две моды:
* мода `default`,  которая отвечает за генерацию элемента БЭМ-дерева вцелом. В рамках этой
моды задан набор и порядок прохождения остальных мод, а также определена
процедура сборки финального BEMJSON из фрагментов, сгенерированных в остальных модах.
* мода `content`, описывающая содержимое текущего элемента БЭМ-дерева.

Создание шаблонов по моде `default` имеет смысл для:
* замены текущей БЭМ-сущности в БЭМ-дереве на другую; 
* создания несколько сущностей в результирующем BEMJSON на одну входную БЭМ-сущность. 

В случаях, когда ни одно из  вышеперечисленных действий не требуется, шаблоны создаются по моде `content`.
...
При таком подходе, взаимодействие с бэендом осуществляет особый модуль – **контролер**. 
...
<tr>
    ...
    <td><code>String</code></td>
    <td>Выполняет асинхронно переданную в качестве аргумента функцию.</td>
</tr>
</table>
