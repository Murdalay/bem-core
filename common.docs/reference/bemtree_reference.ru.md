...
<a name="uts"></a>
####Поддержка UTS
BEMTREE – [UTS-шаблонизатор](http://ru.bem.info/libs/bem-core/2.1.0/template/template#uts_intro). Иначе говоря, BEMTREE использует **унифицированный синтаксис шаблонов** и сохраняет все особенности UTS-шаблонизаторов, такие как:
...
Поскольку BEMTREE основан на JavaScript, в качестве формата представления БЭМ-дерева выбран JSON с набором дополнительных соглашений о представлении БЭМ-сущностей — BEMJSON.
...
Понятия **шаблона, моды и контекста** являются базовыми для UTS и полностью применимы к BEMTREE.
...
BEMTREE-шаблоны создаются с помощью [унифицированного синтаксиса шаблонов](#unity). 
...
Определение шаблона по моде `default` (подпредикат `def()`) необходимо в тех случаях, когда нужно переопределить процедуру генерации выходного фрагмента BEMJSON. Например, следующий BEMTREE-шаблон доопределяет блок `page` с уровня переопределения`common-blocks`:
...
BEMTREE расширяет набор [контекстно-независимых полей UTS](http://ru.bem.info/libs/bem-core/2.1.0/template/template#context_independent) только одним методом – `this._.doAsync`.
...
```js
{
    ...
    users:  { 'login1': { userpic: 'URL', name: 'Full Name 1' }, 'login1': { userpic: 'URL', name: 'Full Name 1' } … },
}
```
