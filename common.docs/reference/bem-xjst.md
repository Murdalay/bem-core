...
###Общие сведения
В `bem-core` реализован механизм двухэтапной шаблонизации данных. На первом этапе нормализованные данные, получаемых от бэкенда, преобразуются в вью-ориентированное представление.  Эту функцию выполняет шаблонизатор BEMTREE. 
...
На втором этапе, ранее сформированный BEMJSON поэлементно преобразуется в HTML. Эту функцию выполняет шаблонизатор BEMHTML. Для каждой БЭМ-сущности создаются HTML-тэги, классы и атрибуты, в соответствие с  BEMHTML шаблонами для этой сущности.
