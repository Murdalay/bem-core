...
Для удобного добавления востребованной функиональности к блокам, используется специальный тип блоков – **блоки-миксы**. Главная особенность блоков-миксов состоит в том, что они не участвуют в цепочке наследования. Это позволяет подмешивать, реализованную в них функциональность к другим блокам, без риска нарушения у последних связей с родительскими блоками (использование статических методов и т.д.).
...
Чтобы подмешать к блоку один или несколько блоков-миксов, нужно присвоить опциональному полю `baseMix` в описании блока значением массив имен примешиваемых блоков-миксов:
...
В качестве блоков-миксов могут быть использованы только блоки, специально созданные для этих целей.
Для декларации таких блоков используется метод `declMix`. Этот метод принимает декларацию блока в формате, аналогичном методу `decl`:
...
**NB** Первым аргументом в декларации блока-микса всегда выступает строка – имя блока. Это связано с тем, что для блоков-миксов не предполагается возможность использования модификаторов или примешивания блоков. 
...
**Важно:** Так как блоки-миксы не участвуют в цепочке наследования, внутри них нельзя использовать `__base`, для обращения к родительскому блоку. 
